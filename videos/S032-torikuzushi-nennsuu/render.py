...
UNITS = [
    ...
    Unit("hikaku", "90歳まで持たせるなら、使えるのは毎月【3万3000円】。", anim=1.2,
         se="don", speed=1.0, intonation=1.2, chara="none"),
    Unit("hikaku", "予定より毎月【1万7000円】、生活費を切り詰めます。", anim=0.0,
         se="impact", se_at=0.25, speed=1.0, intonation=1.2, pad=0.4, chara="none"),

    # ---- 考え方の転換。「いくら使うか」から始めない
    Unit("kime", "毎月の【金額】から決めてはいけません。", anim=1.2,
         speed=1.05, chara="none"),
    Unit("kime2", "【金額】ではなく、何歳まで持たせるかです。", anim=1.2,
         se="don", speed=1.05, chara="none"),

    # ---- 方法。**割るのは「ゴールまでの月数」**(年齢を月数にするのではない)
    Unit("shiki", "そこまでは【300か月】。貯金額を割ります。", anim=1.2,
         speed=1.05, chara="none"),
    Unit("shiki", "こうすれば、貯金が途中で尽きません。", anim=0.0,
         speed=1.05, pad=0.3, chara="none"),

    # ---- 締め。**答えやすい問い**にしてコメントを求める
