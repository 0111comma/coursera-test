...
SCENES = {
    ...
    "ie__cover": sl.cover("変動金利は、何%まで上がったら固定金利に負けるのか",
                          ...
                          main_size=200),
    "mayoi": sl.card("借りる前の分岐", "2つの金利", "※ 途中で乗り換えるには費用がかかる",
                     BADGE, BRAND, main_size=110, head_fs=32),
    "kotae5": sl.hero("5年後なら 3.83%", "※ 総返済額が固定金利と同じになる点", BADGE, BRAND,
                      size=96, sub_fs=30),
    "hitotsu": sl.card("ただし", "答えは1つではない", "※ 借入額・期間でも動く",
    ...
    "minaoshi": sl.card("見直しの回数", "年2回", "※ 反映は金融機関ごとに時期が違う",
                        BADGE, BRAND, main_size=150, head_fs=32),

    # ---- 第2章の追加(5年ルール・125%ルール)
    "gonen": sl.card("よくある安心材料", "5年ルール", "※ 採用しない金融機関もある",
    ...
    "mibarai": sl.card("残るもの", "未払利息", "※ 最後にまとめて請求される場合がある",
                       BADGE, BRAND, main_size=150, head_fs=32),
    "kawaranai": sl.hero("総額は変わらない", "※ 5年ルールは総額を減らす仕組みではない",
    ...
    "hayaku": sl.hero("勝負は前半で決まる", "※ 後半に上がっても効きにくい",
                      BADGE, BRAND, size=100, sub_fs=32),
    ...
    "ch5": sl.chapter(5, "繰上返済", "繰り上げて返すと、分かれ目は動くのか?",
                      BADGE, BRAND),
    "kuriage": sl.card("試すこと", "5年目に300万円", "※ 手数料は無料と仮定",
                       BADGE, BRAND, main_size=88, head_fs=32),
    "kuriage2": sl.barsN("繰上返済のあり・なし",
                         [("そのまま", BE[5], "3.83%"),
                          ("300万円 繰上", BE_PREPAY, "4.26%")],
                         BADGE, BRAND, ymax=5.2, highlight=1),
    "kuriage3": sl.hero("+0.43%", "※ 早く返すほど、この幅は広がる",
                        BADGE, BRAND, size=190, sub_fs=32),

    # ---- 第5章の追加(2つの方式)
    "houshiki": sl.compare2("どちらの方式を選ぶか",
    ...
    "ch6": sl.chapter(6, "借りたあとの人", "すでに変動で借りている人は、どう読むのか?",
                      BADGE, BRAND),
    "sudeni": sl.card("読み替えかた", "残り年数で見る", "※ 借入残高が違えば額も変わる",
                      BADGE, BRAND, main_size=88, head_fs=32),
    "nokori": sl.table("残り年数ごとの目安",
                       ...
                       BADGE, BRAND),
    "hiyou": sl.card("借り換えの前に", "見積もりを取る", "※ 金融機関ごとに大きく違う",
                     BADGE, BRAND, main_size=104, head_fs=32),
    "sonobun": sl.card("足し算がひとつ", "費用ぶんの上乗せ", "※ 表の数字は費用ゼロの前提",
                       BADGE, BRAND, main_size=88, head_fs=32),
    ...
    "kateii": sl.card("表の前提", "一度上がったら続く", "※ 段階的に上がる場合はもっと緩やか",
                      BADGE, BRAND, main_size=88, head_fs=32),
    "zentei2": sl.card("前提その2", "5年ルール", "※ 採用しない金融機関もある",
                       BADGE, BRAND, main_size=120, head_fs=32),
    "zentei3": sl.card("前提その3", "固定金利の水準", "※ 銀行の全期間固定はまた別",
                       BADGE, BRAND, main_size=96, head_fs=32),
...
UNITS = [
    ...
    Unit("genjitsu", "その11.04%や21.90%は、かなり高い水準なのだ。", anim=0.0, speed=1.05),
    Unit("genjitsu", "つまり20年逃げ切ると、届きにくくなるのだ。", anim=0.0,
