...
SCENES = {
    "nazo": sc.hero("配当をもらった日に", "起きること", BADGE, BRAND,
                    size=74, sub_fs=44),
    "nazo__cover": sc.cover("配当をもらうと、なぜ株価が下がる?", "配当落ち",
                            "もらった分だけ、下がる", "仕組みを図で解説", BRAND, main_size=116),
    "chart": sc.price_chart(CHART, [(U_BEFORE, "配当をもらう権利", "sell")], None,
                            "配当をもらう前日と、その翌日", BADGE, BRAND,
                            ymin=2955, ymax=3015, unit="円", reveal=0.52),
    "ochi": sc.price_chart(CHART, [(U_BEFORE, "もらう権利", "sell"), (U_AFTER, "翌日", "buy")],
                           (3000, 2970, "30円 下がる"),
                           "配当をもらう前日と、その翌日", BADGE, BRAND,
    ...
    "zeikin": sc.bars2("税金を引いた後の合計",
                       ("配当をもらう前", 3000, "3000円"),
                       ("もらった直後", 2993.9, "2993円"),
                       BADGE, BRAND, gap="差 6円", ymax=3100),
    "nisa": sc.card("ただしNISAの口座なら", "税金はかからない", "(税金が引かれないぶん、目減りしない)",
                    BADGE, BRAND, main_size=52, head_fs=34,
                    ask="あなたはどの口座?"),
    "imi": sc.card("つまり配当は", "増えたお金ではない", "(会社のお金が、あなたに移っただけ)",
                   BADGE, BRAND, main_size=50, head_fs=34),
    "chui": sc.card("ここまでは計算の話", "その日の市況でも動く", "(いつも同じ幅で下がるわけではない)",
...
UNITS = [
    ...
    Unit("chart", "その日は【権利落ち日】なのだ。", anim=1.2, face="happy",
         speed=1.15, intonation=1.2),
    Unit("ochi", "その3000円の株が、2970円になるのだ。", anim=1.4, face="surprised",
         se="impact", se_at=0.34, speed=1.1, intonation=1.2),
    Unit("onaji", "つまり下がった30円は、配当と同じ額。", anim=1.4, speed=1.15),
    Unit("dokokara", "その配当は、会社のお金が移ってきたもの。", anim=1.4, speed=1.15),
    Unit("toi", "では合計すると、もとどおりなのだ?", anim=1.4, face="troubled",
         speed=1.1, intonation=1.2, pause_scale=1.3),
    Unit("zeikin", "その配当の30円に、【2割の税金】がかかる。", anim=1.4, face="troubled",
         speed=1.15),
    Unit("nisa", "だから合計は2993円。6円減っているのだ。", anim=1.4,
         puchun=True, speed=1.1, intonation=1.2, pitch=-0.05),
    Unit("imi", "つまり配当は、増えたお金ではないのだ。", anim=1.2, speed=1.15),
    Unit("chui", "ただし実際の値動きは、理論どおりではない。", anim=1.2, speed=1.15),
    Unit("shime", "これで、配当落ちのニュースが分かる。", anim=1.0, pad=0.15, face="smug",
