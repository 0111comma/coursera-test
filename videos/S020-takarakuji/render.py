...
SCENES = {
    ...
    "horitsu": sc.card("その宝くじ、実は法律で", "当せん金は5割以下", "(当せん金付証票法で上限が決まってる)", BADGE, BRAND,
                       main_size=48, head_fs=30),
    "yume": sc.card("ここだけの話", "夢の値段は5,350円", "(10,000円 − 戻る4,650円)", BADGE, BRAND,
                    main_size=50),
    "sutansu": sc.card("その夢の値段を知った上で", "買うなとは言わない", "(夢は自由。知って選ぶのだ)", BADGE, BRAND,
                       main_size=50, head_fs=30),
...
UNITS = [
    Unit("hero_count", "【4650円】。宝くじ1万円分の平均の戻り。", anim=1.2, cover=True,
         se="pop", face="normal", speed=1.05, intonation=1.2, pitch=0.0),
    Unit("kangen", "つまり1万円のうち、半分以上は戻らない。", anim=1.6, face="troubled",
         speed=1.1, intonation=1.15, pitch=-0.04),
    Unit("pct", "この戻る割合にすると、46.5%なのだ。", anim=1.2, speed=1.15),
    ...
    Unit("horitsu", "その宝くじは、法律で5割以下と決まってる。", anim=1.2, se="don", speed=1.15),
    Unit("yume", "つまり夢の値段は、【5350円】なのだ。", anim=1.2, face="troubled",
         speed=1.1, intonation=1.3, pitch=0.02),
    Unit("sutansu", "その値段を知った上でなら、買うのは自由。", anim=1.2, face="happy",
