...
REVEAL_WORDS={'shipping':{'hook':'ゴヒャク','cart':'ヨンセンゴヒャク','threshold':'アトセン','reveal':'ゴセン','difference':'ゴヒャク'}}
REVEALS={s['scene_id']:mora_offset(i,REVEAL_WORDS[C['theme']][s['scene_id']]) for i,s in enumerate(S) if s['scene_id'] in REVEAL_WORDS[C['theme']]}
...
def foreground(i):
    ...
    if not s.get('kind'):
        for who in ['maki','zunda']:
            ...
            if cam=='wide':width=382 if who=='maki' else 378;cx=253 if who=='maki' else 826
