...
SCENES = {
    ...
    # ユーザー指摘(ループ63):「これはいつの話なの?」。時点は**声で**言う。
    # バッジは条件の注記であって、耳で聞いている人には届かない(W5ラジオテスト)
    "jiten": sc.card("この動画の前提", "2026年8月", "(金利も手数料も、年で変わる)",
                     BADGE, BRAND, main_size=54, head_fs=34),
    "kinri": sc.card("いまの普通預金は", "年0.4%", "(2026年8月・大手3行)",
                     BADGE, BRAND, main_size=84, head_fs=34),
    "risoku": zandaka(),
    "ritsu": sc.card("利息にかかる税金", "20.315%", "(所得税と住民税の合計)",
    ...
    "atm": sc.card("コンビニで下ろすと", "1回330円", "(大手行の時間外の水準)",
                   ...
                   ask="あなたの口座、無料は月何回?"),
    "itsu": sc.card("かかるのはいつか", "夜と土日", "(平日の昼だけ安い銀行が多い)",
                    BADGE, BRAND, main_size=80, head_fs=34),
...
UNITS = [
    Unit("nazo", "銀行に100万円を預けると、1年で3187円。", anim=1.0, cover=True,
         se="pop", face="normal", speed=1.05, intonation=1.25),
    Unit("jiten", "これは2026年8月の金利で計算した。", anim=1.2, face="happy",
         speed=1.15, intonation=1.2),
    Unit("kinri", "その金利は、いま年0.4%なのだ。", anim=1.2, speed=1.15),
    Unit("risoku", "その0.4%を金額にすると、年4000円になる。", anim=1.4, speed=1.15),
    Unit("ritsu", "ただし利息には、20.315%の税金がつく。", anim=1.4, speed=1.15),
    Unit("hikare", "つまり813円が、引かれるのだ。", anim=1.4, face="troubled",
         speed=1.15),
    Unit("nokori", "だから残るのが、3187円なのだ。", anim=1.6, speed=1.15),
    ...
    Unit("atm", "その手数料はコンビニで、1回330円かかる。", anim=1.4, speed=1.15),
    Unit("itsu", "これは平日の夜や、土日に下ろした時。", anim=1.4, speed=1.15),
    Unit("tsuki", "その330円を、月1回はらうとする。", anim=1.4, speed=1.15),
    Unit("hikaku", "すると1年で3960円になり、利息より多い。", anim=1.6, face="surprised",
         puchun=True, se="don", speed=1.1, intonation=1.2),
    Unit("erabenai", "しかも金利を、自分では選べないのだ。", anim=1.4, face="troubled",
         speed=1.15),
    # ユーザー指摘(ループ61):「減らすのを止める」だけでは何をすればいいか分からない。
    # 具体案を3つ、チェックリストとして上から点灯させる
    Unit("herasu0", "でも手数料なら、3つの方法で減らせる。", anim=1.4, face="happy",
         speed=1.15, intonation=1.2),
    Unit("herasu1", "まず、下ろすのは平日の昼にする。", anim=1.4, speed=1.15),
    Unit("herasu2", "そして下ろす回数を、月1回にまとめる。", anim=1.4, speed=1.15),
    Unit("herasu3", "さらに、無料回数を調べておくのだ。", anim=1.4, speed=1.15),
