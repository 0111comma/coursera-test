...
SCENES = {
    "nazo": sc.hero("株は元に戻ったのに", "減った人がいる", BADGE, BRAND,
                    size=76, sub_fs=44),
    "nazo__cover": sc.cover("株が元に戻ったのに、なぜ減る?", "2倍の商品",
                            "毎日リセットされるから", "はじめての人向け", BRAND, main_size=118),
    "namae": sc.card("何を買っていたのか", "2倍で動く商品",
                     "(レバレッジ型と呼ばれる)", BADGE, BRAND,
                     main_size=54, head_fs=34),
    "setsumei": sc.card("2倍で動く商品とは", "株全体の平均の2倍",
                        "(日経平均などの平均のこと)", BADGE, BRAND,
                        main_size=48, head_fs=34),
    "yakusoku": sc.card("ただしその2倍は", "1日ぶんだけの約束", "(毎日、はかり直す)",
    ...
    "nijuu": sc.bars2("1日目の値動き",
                      ...
                      BADGE, BRAND),
    "nokori": scene_maiban,
    "day2": sc.card("次の日、平均は元へ", "もどるには何%必要?", "(90から100に戻す上げ幅)",
                    BADGE, BRAND, main_size=46, head_fs=34,
                    ask="あなたなら、何日持つ?"),
    "juuichi": sc.card("その答えは", "11.1%上がる", "(90 × 1.111 = 100)",
    ...
    "shime": sc.hero("上下するほど", "2倍の商品は弱い", BADGE, BRAND, size=92, sub_fs=42),
}

# ネタ選定ゲート(F1): 予想「2倍の商品なら、平均が戻れば自分も戻るのでは?」
#   → 結論「毎日リセットされるので、上下をくり返すと元の水準に戻らない」
# ループ54(文脈): 1文ずつ、直前の文に出た語を必ず受ける。
#   接続語を置くだけでは繋がらない(S013で実際にそれをやって却下された)
UNITS = [
    Unit("nazo", "株の値段は元に戻ったのに、減った人がいる。", anim=1.0, cover=True,
         se="pop", face="normal", speed=1.05, intonation=1.25),
    Unit("namae", "その人が買ったのは、【2倍で動く商品】。", anim=1.2, face="happy",
         speed=1.15, intonation=1.2),
    Unit("setsumei", "その商品は、株の平均の2倍だけ動く。", anim=1.4, speed=1.15),
    Unit("yakusoku", "ただしその2倍は、1日ぶんの約束。", anim=1.4, speed=1.15),
    Unit("kau", "その商品を、100だけ買うのだ。", anim=1.4, speed=1.15),
    Unit("day1", "すると1日目に、平均が10%下がった。", anim=1.4, speed=1.15),
    Unit("nijuu", "その2倍なので、商品は20%下がるのだ。", anim=1.4, speed=1.15),
    Unit("nokori", "つまり100が、80になるのだ。", anim=1.4, face="troubled", speed=1.15),
    Unit("day2", "では次の日、平均が元にもどる。", anim=1.4, face="troubled",
         speed=1.1, intonation=1.2, pause_scale=1.3),
    Unit("juuichi", "その平均がもどるには、【11.1%】必要。", anim=1.4, speed=1.15),
    Unit("kekka", "その2倍でも、80からでは足りない。", anim=1.6, face="troubled",
         speed=1.15),
    Unit("sa", "この商品は【97.8】。平均は100なのに。", anim=1.4, face="surprised",
         puchun=True, se="impact", se_at=0.34, speed=1.1, intonation=1.2, pitch=-0.05),
    Unit("riyu", "この差は、80から増やしたから。", anim=1.2, speed=1.15),
    Unit("joken", "逆に上げ続ける時は、大きく増える。", anim=1.2, face="happy",
         speed=1.15),
    Unit("shime", "上下をくり返すほど、目減りは積もるのだ。", anim=1.0, pad=0.15, face="smug",
