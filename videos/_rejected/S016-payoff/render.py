...
SCENES = {
    ...
    "kotae": sc.reveal("1000万円+利息", "1つの銀行・1人あたり、ここまで保護", "「預金保険」という国の仕組み", BADGE, BRAND,
                       size=88),
    "seido": sc.card("この仕組みの名前は", "国の預金保険", "(銀行がお金を出し合う保険の仕組み)", BADGE, BRAND,
                     main_size=54),
    "jitsurei": sc.card("この預金保険、実話がある", "2010年に発動した", "(日本振興銀行の破綻。戦後唯一)", BADGE, BRAND,
                        main_size=52, head_fs=30),
    "hamidashi": scene_hamidashi,
    "kessai": sc.card("実は", "利息ゼロの口座なら全額", "(「決済用預金」は上限なしで保護)", BADGE, BRAND,
                      main_size=50,
                     ask="あなたの貯金は、上限の内側?"),
    "gaika": sc.card("逆に", "外貨預金は対象外", "(円の預金とはルールが別)", BADGE, BRAND,
                     main_size=54),
...
UNITS = [
    Unit("hero_full", "【1000万円】。銀行が潰れても守られる貯金。", anim=1.2, cover=True,
         se="pop", face="normal", speed=1.05, intonation=1.2, pitch=0.0),
    Unit("tsusetsu", "その銀行が潰れたら、全部消えると思ってた?", anim=1.4, face="troubled",
         speed=1.1, intonation=1.2),
    Unit("kotae", "実は1つの銀行で1人、【1000万円】まで守られる。", anim=1.4, face="surprised",
         puchun=True, se="impact", se_at=0.34,
         speed=1.1, intonation=1.2, pitch=-0.05, pause_scale=1.3),
    Unit("seido", "この仕組みの名前が、国の【預金保険】なのだ。", anim=1.2, speed=1.15),
    Unit("jitsurei", "この預金保険、2010年に発動したのだ。", anim=1.2, speed=1.15),
    Unit("hamidashi", "では1500万円だと?500万円は、はみ出す。", anim=1.6, face="troubled",
         speed=1.1, intonation=1.15, pitch=-0.04),
    Unit("kessai", "ただし利息ゼロの口座なら、【全額】保護。", anim=1.2, se="don", speed=1.15),
    Unit("gaika", "逆に外貨預金は、この保護の対象外なのだ。", anim=1.2, face="troubled", speed=1.15),
    Unit("meyose", "そして同じ銀行なら、口座を分けても1000万。", anim=1.2, speed=1.15),
