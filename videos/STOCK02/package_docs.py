...
- 音声生成はGitHub Actionsで成功。合成かなを全区間点検し、金額の説明は聞き取れる速さに調整。演技区間には個別の声色・緩急・間を設定。口は最終音声のフレームごとの音量に同期。
...
- 感情表現の修正：下落時は叫び→なみだめの焦り→ツンツンの売却へ切替。安堵と再度の落胆にも声の差をつけ、結末はヘロヘロ・低めの声・語尾の下降・間を使用。BGMを下げ、明るい終盤SEと跳ねを修正。
- 演技の区間ごとに音声を合成し、実サンプル長から字幕と数字を同期。実使用スタイルと発音情報はクエリに記録。
