...
金融ショートのストック2。完成49.67秒／1080×1920／30fps。VOICEVOXずんだもん・四国めたん。
...
`script.json`の`delivery`で、同じ台詞内でも声色・速さ・間を個別指定できる。`performance.py`は稼働中のVOICEVOXの`/speakers`からずんだもんのスタイル名を解決し、各区間を個別合成する。叫びの母音長とピッチ、落胆した語尾の下降を調整。各区間の実サンプル長から`timed_moras`を作り、字幕・数値・効果音の同期に使う。この複合クエリは時刻管理用で、VOICEVOXへ直接再送しない。`retime.py`は演技済み区間の緩急を保つ。
