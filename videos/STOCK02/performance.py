...
def style_id(name, request, output):
    global STYLE_CACHE
    if STYLE_CACHE is None:
        ...
        zunda = next(s for s in cast if s['name']=='ずんだもん')
        STYLE_CACHE = {s['name']:s['id'] for s in zunda['styles']}
...
def synthesize_performance(scene, request, output):
    ...
    for beat in scene['delivery']:
        speaker=style_id(beat.get('style','ノーマル'),request,output)
