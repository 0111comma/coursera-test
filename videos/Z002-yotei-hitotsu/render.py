...
SCENES = {
    ...
    "hantei": sz.calendar_one("02_point", "今週", "?", bubble="多すぎるのは?"),
    ...
    "migiude": sz.emperor("04_surprised", "皇帝ネロ", bubble="家庭教師だった"),
...
UNITS = [
    ...
    U("tarinai", "時間はちゃんとある。多すぎるのは予定のほうだよ。"),
    U("hantei", "予定に時間を渡してるのは、自分だよ。", **_S),
    U("joushi", "上司のせいじゃない。自分がいいよって入れた予定だよ。"),
    U("watashi", "予定を入れたのも自分。時間を渡してるのも自分。", **_D),
    U("toi2", "じゃあ、やめる1件はどう選ぶ? ローマの役人あての本にあるの。"),

    # ---- 幕2 誰が言った(8)。**宛先の職**が、視聴者の反論(自分がいないと回らない)への答え
    U("dashita", "忙しすぎる役人に、仕事は手放せって書いた人がいるの。"),
    U("seneca", "役人あてに書いたのがセネカ。ストア派の哲学者ね。"),
    U("paulinus", "役人の名はパウリヌス。ローマじゅうの食料をあずかる長官。"),
    U("uekomi", "あなたの会議と違って、食料が止まれば街が飢えるの。"),
    U("uekomi2", "食料の職なら、自分がいないと本当に回らないよね。", **_S),
    U("kaite", "その回らない職の人に、セネカは手放せって書いたの。"),
    U("migiude", "セネカも、皇帝ネロの家庭教師だった人。", **_S),
    U("migiude2", "ネロが即位してからは、その右腕。あなたの上司どころじゃないの。"),
    U("hon", "右腕になったセネカが書いた本が、『人生の短さについて』。"),

    # ---- 幕3 引用(6)。1.1 は「人生は短い」と嘆く側から始まり、1.3 でセネカが返す
    U("hajimari", "本の出だしは、人生は短いって嘆く人の話。あなたと同じ。"),
    U("kaesu", "嘆きに、人生は短くないってセネカが返すの。", **_S),
    U("quote1", "「人生が短いんじゃない。多くを浪費してるんだ」。"),
    U("dare", "浪費してるのは自分。減っていくのは、自分の人生なの。", **_D),
    U("occupati", "浪費って、他人の用事で埋まった人生の一日のことね。"),
    U("quote1b", "「短い人生をもらったんじゃない。自分たちで短くしたんだ」。"),

    # ---- 幕4 なぜ残った(4)。**how ではなく why**(修道院が写す本に選び続けた理由)
    U("nokotta", "人生の短さについての本が、いまも読めるのはなんで?"),
    U("shahon", "答えは、印刷が無い時代。修道院で写された本だけが残るの。"),
    U("miuchi", "その修道院で、異教徒のセネカはほとんど身内あつかい。"),
    U("todoita", "だからセネカだけは写され続けて、あなたにも届いたの。"),

    # ---- 幕5 誰が評価した(3)。**予定を実際に消した実例**
    U("yonda", "読んだ1人が、16世紀のモンテーニュ。活字のセネカね。"),
    U("yameta", "モンテーニュは37歳で、裁判所の職ごと手放したの。"),
    U("chuui", "そこまでしろとは言わないの。あなたは1件でいい。", **_S),

    # ---- 幕6 もう一文 → 動作 → 締め(8)。**山は動作のカットに置く**
    U("iu2", "1件をどう選ぶかは、セネカの財産の話にあるの。", **_S),
    U("zaisan", "あなたの財産、誰かにぽんと分けられる?"),
    U("quote2", "「財産なら、誰にも分け与えないだろう」。", **_S),
    U("waketa", "なのに人生の時間なら、平気で分け与えるって書いてるの。", **_S),
    U("kesu1", "あなたの今週も、分け与えた時間で埋まってない?"),
    U("kesu2", "出なくていい1件を選んで、今週のうちに消す。", **_D),
    U("henshin", "自分で消せない会議なら、今週じゅうに「欠席します」って返事して。"),
    U("owari", "返事は一言。今週の予定の1件ぶんは、もう人に分けずにすむの。"),
]
