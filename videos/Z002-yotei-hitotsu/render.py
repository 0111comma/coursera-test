...
UNITS = [
    ...
    U("gisshiri", "時間がないんじゃなくて、予定が多すぎるんだよ。"),
    U("hantei", "予定を入れたの、自分だよね。", **_S),
    U("joushi", "その予定、上司が勝手に入れた? 自分でいいよって言ったよね。", **_D),
    U("toi2", "じゃあどれをやめる? 昔の本に答えがあるの。"),

    # ---- 幕2 誰が言った(8)。**宛先の職**が、視聴者の反論(自分がいないと回らない)への答え
    U("dashita", "その本、あなたより忙しい役人に手放せって書いてあるの。"),
    U("seneca", "手放せって書いたのはセネカ。ローマのストア派の哲学者ね。"),
    U("paulinus", "パウリヌスって人に書いたの。ローマの食糧長官ね。"),
    U("uekomi", "食糧長官は、街の食べ物をあずかる人のことね。"),
    U("uekomi2", "食糧長官がいなくなったら、街が飢えるでしょ?", **_S),
    U("migiude", "そのセネカ、暇な人だと思う? 逆で、皇帝ネロの先生ね。", **_S),
    U("migiude2", "ネロが皇帝になってからは政治顧問。上司より忙しい人だよ。"),
    U("hon", "政治顧問が書いた本が、『人生の短さについて』。"),

    # ---- 幕3 引用(6)。1.1 は「人生は短い」と嘆く側から始まり、1.3 でセネカが返す
    U("hajimari", "本の出だしは、人生は短いって嘆く人の話。あなたと同じ。"),
    U("kaesu", "それにセネカが、人生は短くないよって返すの。", **_S),
    U("quote1", "「人生が短いんじゃない。多くを浪費したんだ」。"),
    U("dare", "浪費してるのは自分。使いつぶしてるのは、自分の人生。", **_D),
    U("occupati", "人生を人の用事でつぶす人を、セネカは多忙な人って呼ぶの。"),
    U("quote1b", "「短い人生をもらったんじゃない。自分たちで短くしたんだ」。"),

    # ---- 幕4 なぜ残った(4)。**how ではなく why**(修道院が写す本に選び続けた理由)
    U("nokotta", "てか、セネカの一言がなんであなたに届いてるの?"),
    U("shahon", "だって昔は印刷が無いから、修道院で手で写した写本だけ残るの。"),
    U("miuchi", "その写本を作ってた修道院で、セネカは身内あつかいだったの。"),
    U("todoita", "キリスト教じゃないのに身内あつかい。だから写され続けたの。"),

    # ---- 幕5 誰が評価した(3)。**予定を実際に消した実例**
    U("yonda", "写本を読んだのがモンテーニュ。フランスの役人ね。"),
    U("yameta", "モンテーニュは37歳で職を手放して、塔でセネカを読んだの。"),
    U("chuui", "セネカを読んで、職ごと消した人ね。あなたは1件でいいよ。", **_S),

    # ---- 幕6 もう一文 → 動作 → 締め(8)。**山は動作のカットに置く**
    U("iu2", "で、やめる1件はどう選ぶ? セネカのお金の話がヒントなの。", **_S),
    U("zaisan", "あなたのお金、誰かにぽんと分け与えられる? 無理だよね。"),
    U("quote2", "「お金なら、誰にも分け与えないだろう」。", **_S),
    U("waketa", "なのに時間だと、平気で人に分け与えちゃうよね。", **_S),
    U("kesu1", "自分が一度も発言しない会議。それが分け与えた時間ね。"),
    U("kesu2", "会議を1件選んで、今週のうちに消す。", **_D),
    U("henshin", "自分で消せない会議なら、「欠席します」の返事でいい。"),
