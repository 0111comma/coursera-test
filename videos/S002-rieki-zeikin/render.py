...
def scene_hayami(fig, t):
    ...
    for i, (g, tax) in enumerate(zip(TABLE_GAINS, TABLE_TAX)):
        ...
        fig.text(0.76, y, "0円", ha="center", color=GOLD, fontsize=30)
    fig.text(0.46, 0.425, "税率20.315%で計算(2026年8月時点)", ha="center", color=MUTED, fontsize=20)
    draw_badge(fig, "2026年8月時点の税制・制度")
    draw_footer_brand(fig, BRAND)
...
UNITS = [
    ...
    Unit("stack_base", "月1万円を20年、積み立てたとするのだ。", anim=1.8, speed=1.15),
    ...
    Unit("quiz", "171万円の20%…【いくら】だと思うのだ?", anim=1.4,
         speed=1.15, intonation=1.25),
    Unit("answer", "答えは、【34万7千円】なのだ。", anim=1.2, se="don",
         speed=1.1, intonation=1.2, pitch=-0.06, pause_scale=1.4),
    Unit("tedori", "手取りは、【136万円】なのだ。", anim=1.4,
         speed=1.1, intonation=1.1, pitch=-0.06),
    Unit("breakdown", "正確には、【20.315%】なのだ。", anim=2.0, speed=1.1, intonation=1.1),
    Unit("nisa_zero", "でも【NISA】なら、この税金がゼロ。", anim=1.8,
         narration="でもニーサなら、この税金がゼロ。",  # エヌアイエスエー読み回避(kana照合済み)
         speed=1.15, intonation=1.25, pitch=0.0),
    Unit("marugoto", "同じ利益でも、手取りは【171万円】。", anim=1.2, speed=1.15, intonation=1.2),
    Unit("chishiki", "腕の差じゃなく、【知識の差】なのだ。", anim=1.2, pad=0.35,
    ...
    Unit("waku", "NISAの非課税枠は、1人【1800万円】。", anim=1.2,
         narration="ニーサの非課税枠は、1人1800万円。",
