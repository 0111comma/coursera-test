...
- Q4(自分でできるか)は3人とも「最初の一歩で止まる」。自分の厚生年金の額の
  ...
  → **締めを「あなたの厚生年金は? → ねんきん定期便に → その額に給料を足す・
    ボーナスは12で割る → 65万円以下なら減らない → 抑えなくていい」の5カット**に
    した。判定だけ渡して終わらせない
...
**幕5は幕3と同じ形に揃えた**(2026-08-31 の追い直し)。
Q5 で3人全員が「9万5000円がどこから出たか分からないまま書き取った」と
書いたのは、幕3では「超過を出す → その半分」の2手を見せたのに、
幕5だけ落差に差し替えたためだった。**手を減らすのではなく、同じ手を
もう一度あてる**ことにした:

    幕3  9 合計【55万円】 → 10 51万円を【4万円】超え → 11 その半分は?月【2万円】
    幕5 16 合計【70万円】 → 17 65万円を【5万円】超え → 18 その半分だけ、月【2万5000円】

2度目は言い回しまで同じなので、音だけで追える。5万円は verify.py の
OVER_HIGH(70万−65万)で、図(koeru3)にも出る。
...
- 9万5000円(改正前の減額)と、7万円(その落差)。**幕5で見せる計算を
  4月以降の1本に絞った。**2つ見せると幕3で教えた規則がどちらに
  かかるのか分からなくなる。改正前との差は概要欄に置いた
...
SCENES = {
    ...
    # 幕3(koeru)と同じ形の式をもう一度あてる。**規則の再適用を目で見せる**
    "koeru3": sf.formula("70万円 − 65万円", name=None,
                         answer="5万円", title="4月からの超過"),
    ...
    "teikibin": sf.hero("ねんきん定期便", "毎年、誕生月に届く", name=None,
                        role="neutral", count=False, size="reference"),
    # 図だけが持つ情報: **足すほうの「給料」の中身**。
    # 声は「12で割って足す」までしか言えないので、月給とボーナスの2項は図が持つ
    "bonus": sf.formula("月給 + ボーナス÷12", name=None,
                        answer="これが給料", title="足すほうの中身"),
...
UNITS = [
    ...
    Unit("tasu", "まずあなたの厚生年金と給料を合計する。", anim=1.0,
         speed=1.0, intonation=1.15, chara="none"),
    Unit("kijun", "合計が減りはじめる額は、月【51万円】。", anim=1.0, se="don",
         speed=1.0, intonation=1.2, chara="none"),
    Unit("koeta", "合計がこれを超えた月だけ、厚生年金が減る。", anim=1.0, speed=1.05,
         chara="none"),
    ...
    Unit("rei_a", "給料を足すと合計【55万円】。", anim=1.0, speed=1.05,
         intonation=1.15, chara="none"),
    Unit("koeru", "合計は51万円を【4万円】超え。", anim=1.0, speed=1.05,
         intonation=1.1, chara="none"),
    Unit("tomaru", "その半分は?月【2万円】が減る。", anim=1.0, se="impact",
         se_at=0.1, speed=1.0, intonation=1.25, chara="none"),

    # --- 幕4 転回。**現在形で言い切る**(「もう」で施行済みを渡す)
    Unit("kaisei", "実は2026年4月、法律が変わった。", anim=1.0, speed=1.0,
         intonation=1.15, chara="none"),
    Unit("sa", "法律で51万円が、もう月【65万円】。", anim=1.2, se="don",
         speed=1.0, intonation=1.25, chara="none"),
    Unit("zero", "55万円は65万円より下。減る額【0円】。", anim=1.2, se="don",
         speed=1.0, intonation=1.3, pad=0.35, chara="none"),

    # --- 幕5 正直に言う。全員が0円になるわけではない
    Unit("rei2", "同じ人の給料が月【60万円】なら?", anim=1.0, speed=1.0,
         intonation=1.25, chara="none"),
    Unit("koeru2", "給料を足すと合計【70万円】。", anim=1.0, speed=1.05,
         intonation=1.15, chara="none"),
    Unit("koeru3", "合計は65万円を【5万円】超え。", anim=1.0, speed=1.05,
         chara="none"),
    Unit("mada", "その半分だけ、月【2万5000円】減る。", anim=1.0,
    ...
    Unit("teikibin", "厚生年金はねんきん定期便に。", anim=1.0, speed=1.05,
         chara="none"),
    Unit("bonus", "その額に給料を足す。ボーナスは12で割る。", anim=1.0,
         speed=1.05, chara="none"),
    Unit("tashizan", "合計が月【65万円】以下なら減らない。", anim=1.0, speed=1.05,
         intonation=1.15, chara="none"),
    Unit("cta", "年金のため働く時間を抑えなくていい。", anim=1.0, speed=1.0,
