...
SCENES = {
    ...
    "hanron": sz.book_now("03_troubled", "『哲学書簡』", bubble="1章まるごと"),
...
UNITS = [
    ...
    U("ochitsukanai", "なのにスマホを置いてくと、なんか落ち着かないの。"),
    U("sei", "落ち着かないのは、スマホのせいだと思ってる?"),
    U("jikan", "スマホのせいじゃない。何もしない時間のせいだよ。"),
    ...
    U("kari3", "ほしいのは獲物じゃない。追いかけるほうだから。", **_S),
    ...
    U("nigesaki", "スマホは逃げ先。原因は別にあるよ。", **_S),

    # ---- 幕5 誰が評価した(5)。plan §1.7 の上限は3。**2カット超える理由**:
    # 2026-09-08 U22: 直前にあった「なぜ残った」8カット(ポール・ロワイヤル版・
    # 削った言葉・ラフュマ版)を丸ごと落とした。ユーザー「出典がなぜ残ってるのかな話
    # 必要かな? 面白くない箇所増やさなくていいよ」。伝来は plan.md に残す。
    # **ヴォルテールは残す**。反論は「借りていい見方なのか」を視聴者が決める材料で、
    # 書誌と違って今夜の行動に効くため
    #      反論の中身を言わないと「有名人が一章ぶん反対した」だけの来歴の雑学になり、
    #      視聴者に「で、どっちが正しいの? 自分は借りていいの?」が残る(批評パネル high)
    U("voltaire2", "スマホは逃げ先って話に、噛みついた人がいるの。誰?"),
    U("hanron", "答えは、ヴォルテールって作家。1734年に一章まるごと反論に使うの。"),
    ...
    U("owari", "またスマホでまよっても、自分を責めなくていい。"),
]
