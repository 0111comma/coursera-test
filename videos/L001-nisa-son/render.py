...
S.use_duo()            # ← 二人会話(ずんだもん×四国めたん。duo-skit-2026-08.md)
...
SCENES = {
    ...
    "rei2": sl.compare2("ある1年を切り出すと",
                        ...
                        note_l="※ 差し引きできる", note_r="※ 差し引きできない"),
    ...
    "keisan2": sl.band("引く相手がいないと、こうなる", "差し引きなしの課税対象", 0.79685,
                       "手元 15万9370円", "税 40,630円", BADGE, BRAND, show_rest=True),
    ...
    "uw_a": sl.barsN("3つを足すと20.315%",
                     [("所得税", 15, "15%"), ("住民税", 5, "5%"),
                      ("復興特別所得税", 0.315, "0.315%")],
                     BADGE, BRAND, ymax=17, highlight=0),
    "uw_b": sl.barsN("3つを足すと20.315%",
                     [("所得税", 15, "15%"), ("住民税", 5, "5%"),
                      ("復興特別所得税", 0.315, "0.315%")],
                     BADGE, BRAND, ymax=17, highlight=1),
    "uw_c": sl.barsN("3つを足すと20.315%",
                     [("所得税", 15, "15%"), ("住民税", 5, "5%"),
                      ("復興特別所得税", 0.315, "0.315%")],
                     BADGE, BRAND, ymax=17, highlight=2),
    "nokoru": sl.barsN("同じ年の利益と損をならべる",
                       [("利益", 20, "20万円"), ("損", 20, "20万円"),
                        ("残り", 0.001, "0円")],
                       BADGE, BRAND, ymax=24, highlight=2),
    "amari": sl.barsN("使えた分と、余った分",
                      [("使えた分", 10, "10万円"), ("余った損", 20, "20万円")],
                      BADGE, BRAND, ymax=24, highlight=1),
    "nikai_a": sl.barsN("2年つづいた場合",
                        [("1年目の差", 40630, "40,630円")],
                        BADGE, BRAND, ymax=95000),
    "nikai_b": sl.barsN("2年つづいた場合",
                        [("1年目の差", 40630, "40,630円"), ("2年目の差", 40630, "40,630円")],
                        BADGE, BRAND, ymax=95000, highlight=1),
    "jibun2_a": sl.barsN("ならべて、小さいほうを取る",
                         [("NISAの損", 4, "4万円")], BADGE, BRAND, ymax=14),
    "jibun2_b": sl.barsN("ならべて、小さいほうを取る",
    ...
    "tsugi": sl.card("1分で見たいとき", "税金の20.315%", "※ 忙しい人向け",
                     BADGE, BRAND, main_size=88, head_fs=32),
...
UNITS = [
    # ===== 冒頭(0〜約45秒) — 二人会話版(duo-skit-2026-08.md)
    #
    # 役割: ずんだもん=聞き手(視聴者の代弁・speaker省略=3) / めたん=解説役(speaker=2)。
    # 規則: めたんの連続台詞は3つまで / リビール直後はずんだもんの反応 /
    #       章の入口の問いはずんだもんが言う / face はずんだもんの表情
    #       (めたんの台詞中でも、聞いているずんだもんの反応として使う)。
    # 順序の設計(得の話→NISAの例外)は一人語り版と同じ。ループ71のユーザー指摘:
    #   「他の株で損したら儲かった株の税金を払わなくていいことを基本的に人は知らない」
    Unit("son", "株で損したら、税金はどうなるのか?", anim=1.4, cover=True,
         se="pop", speed=1.05, intonation=1.3),
    Unit("son", "たとえば株を売って、20万円の損が出たのだ。", anim=0.0,
         face="troubled", speed=1.05, intonation=1.25),
    Unit("son", "その損、消えるだけだと思っていない?", anim=0.0,
         speaker=2, face="troubled", speed=1.1, intonation=1.2),
    Unit("sousai0", "では同じ年に、別の株でもうけていたとする。", anim=1.4,
         speed=1.05, chara="none"),
    Unit("sousai1", "そのもうけには、ふつう40630円の税金がかかるの。", anim=1.4,
         speaker=2, face="troubled", speed=1.05, chara="none"),
    Unit("sousai2", "ところが損と差し引くと、40630円が消えるのよ。", anim=1.6,
         speaker=2, se="impact", se_at=0.32, speed=1.0, intonation=1.3,
         face="surprised", chara="none"),
    Unit("reigai", "ただし、その差し引きが使えない口座もあるの。", anim=1.4,
         speaker=2, face="troubled", speed=1.1),
    Unit("sorenisa", "その口座の名前が、NISAなのよ。", anim=1.4,
         speaker=2, se="don", speed=1.0, intonation=1.25, face="surprised"),
    Unit("sorenisa", "そしてNISAでないほうの口座を、課税口座と呼ぶのよ。", anim=0.0,
         speaker=2, speed=1.05),
    Unit("rei2", "えっ、NISAで損した年は40630円を払うのだ?", anim=1.4,
         face="surprised", speed=1.05, chara="none"),
    Unit("yotei", "そこで4つの章で、条件を確かめていくわ。", anim=1.4,
         speaker=2, face="happy", speed=1.15, pad=0.6),
    Unit("rei2", "まず消せたはずの40630円を、覚えておくのだ。",
         anim=0.0, speed=1.05, intonation=1.2, chara="none"),

    # ===== 第1章 引けない損
    Unit("ch1", "では第1章。値段の出どころを、確かめていくわよ。", anim=1.4,
         speaker=2, se="pop", speed=1.1),
    Unit("zeiritsu", "まず株や投資信託の利益には、20.315%かかるの。", anim=1.4,
         speaker=2, speed=1.1),
    Unit("zeiritsu", "投資信託とは、たくさんの銘柄をまとめた商品のことね。", anim=0.0,
         speaker=2, speed=1.05),
    Unit("zeiritsu", "投資信託でも、同じ率なのだな。", anim=0.0,
         speed=1.1),
    Unit("uchiwake", "そして20.315%を分けると、所得税と住民税と復興特別所得税になるのよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("uchiwake", "うち所得税と住民税で、20%を占めるの。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("uw_a", "まず所得税が、15%。", anim=1.4, speaker=2, speed=1.1, chara="none"),
    Unit("uw_b", "そして住民税が、5%なのだ。", anim=1.4, speed=1.1, chara="none"),
    Unit("uw_c", "さらに残りの0.315%が、復興特別所得税なのだ。", anim=1.4,
         speed=1.1, chara="none"),
    Unit("keisan1", "だから20万円の利益なら、税金は40630円になるの。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("keisan1", "そして手元に残るのは、15万9370円なのだ。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("kouza", "ちなみに課税口座には、特定口座と一般口座があるの。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("gensen", "そして特定口座には、源泉徴収ありという選び方があるわ。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("gensen", "源泉徴収ありとは、証券会社が代わりに納めてくれることよ。", anim=0.0,
         speaker=2, speed=1.1),
    Unit("tsuusan", "ではもし、同じ年に損も出ていたらどうなるのだ?", anim=1.4,
         face="troubled", speed=1.05),
    Unit("tsuusan", "その損は、利益から引けるの。", anim=0.0, speaker=2, speed=1.05),
    Unit("sashihiki", "そして引くことを、損益通算と呼ぶのよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("sashihiki", "たとえば利益20万円と損20万円が、同じ年にあるとするわね。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("nokoru", "その差し引きの後に残るのは、0円なのだ。", anim=1.4,
         speed=1.05, chara="none"),
    Unit("nokoru", "そうよ。だから税金も、0円になるの。", anim=0.0,
         speaker=2, se="pop", face="happy", speed=1.05, chara="none"),
    Unit("nokoru", "課税口座では、損が税金を消す材料になるのよ。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("tsuusan2", "ただし引けるのは、同じ年の中だけ。", anim=1.4,
         speaker=2, speed=1.1),
    Unit("hikenai", "ではNISAの損は、どう扱われるのだ?", anim=1.4,
         face="troubled", speed=1.05),
    Unit("hikenai", "答えは、引けない。", anim=0.0,
         speaker=2, se="impact", speed=1.0, intonation=1.3, face="surprised"),
    Unit("riyuu2", "でも、引けない損はどこへ行くのだ?", anim=1.4,
         face="troubled", speed=1.05),
    Unit("riyuu2", "その損は、無いものとされるの。", anim=0.0, speaker=2, speed=1.05),
    Unit("riyuu2", "そして損を引けないのは、利益に税金をかけないからよ。", anim=0.0,
         speaker=2, speed=1.05),
    Unit("riyuu2", "税金を取らないかわりに、損も見ないのだな。", anim=0.0,
         speed=1.05),
    Unit("keisan2", "そして課税口座の利益20万円だけが、まるごと残るのよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("keisan2", "利益にかかる税金が、40630円なのだ。", anim=0.0,
         speed=1.05, chara="none", pad=0.6),
    Unit("rei2", "つまり引けなかった損の値段が、40630円というわけ。", anim=1.4,
         speaker=2, se="don", speed=1.05, chara="none"),

    # ===== 第2章 翌年もう一度
    Unit("ch2", "では第2章。同じ額を、翌年もう一度払う話をするわ。", anim=1.4,
         speaker=2, se="pop", speed=1.1, face="troubled"),
    Unit("kurikoshi", "まず課税口座には、繰越控除という仕組みもあるの。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("kurikoshi2", "繰越控除とは、損を来年に持ち越すことなのだ?", anim=1.4,
         face="happy", speed=1.05),
    Unit("nen1", "そのとおり。たとえば1年目に、40万円の損が出たとするわ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("nen1", "2年目には、40万円の利益を出したとするのよ。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("nen2", "課税口座なら、1年目の損を2年目から引けるの。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("nen2", "だから2年目の税金も、0円になるのだ。", anim=0.0,
         se="pop", face="happy", speed=1.05, chara="none"),
    Unit("san1", "そして持ち越せるのは、3年ぶんまでよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("san1", "たとえば毎年10万円の利益なら、3年かけて引いていくの。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("san2", "でも3年を過ぎると、余った損はもう引けないのよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("san2", "だから引き切れるかは、その後の利益しだいなのだな。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("shinkoku", "ただし繰越控除には、確定申告が要るの。", anim=1.4,
         speaker=2, face="troubled", speed=1.05),
    Unit("shinkoku", "その申告は、損が出た年から毎年出し続けるのよ。", anim=0.0,
         speaker=2, speed=1.05),
    Unit("shinkoku", "だから一度でも出し忘れると、権利を失うわ。", anim=0.0,
         speaker=2, face="troubled", speed=1.05),
    Unit("nen3", "ではNISAの損は、来年に持ち越せるのだ?", anim=1.4,
         face="troubled", speed=1.05, chara="none"),
    Unit("nen3", "その答えも、持ち越せない。", anim=0.0,
         speaker=2, se="impact", speed=1.0, chara="none"),
    Unit("nen3", "だから2年目の利益40万円に、81260円かかるの。", anim=0.0,
         speaker=2, face="troubled", speed=1.05, chara="none"),
    Unit("nikai_a", "では毎年おなじことが起きると、どうなるのだ?", anim=1.4,
         speed=1.05, chara="none"),
    Unit("nikai_a", "まず1年目の差が、40630円。", anim=0.0,
         speaker=2, speed=1.1, chara="none"),
    Unit("nikai_b", "2年目の差も、40630円。", anim=1.4,
         speaker=2, speed=1.1, chara="none"),
    Unit("nikai", "だから合わせると、81260円になるのだ。", anim=1.4,
         speed=1.05, chara="none", pad=0.6, face="surprised"),
    Unit("nikai", "81260円は、消せたはずの税金の2倍なのよ。", anim=0.0,
         speaker=2, se="don", speed=1.05, chara="none"),

    # ===== 第3章 差の上限
    Unit("ch3", "では第3章。差の上限を、額を動かして見るわよ。", anim=1.4,
         speaker=2, se="pop", speed=1.1),
    Unit("onaji", "まず今までの例は、損も利益も同じ額だったの。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("onaji", "次に、損と利益がちがう額の年を見るのだな。", anim=0.0,
         speed=1.05),
    Unit("pat1", "たとえば損が30万円で、利益が10万円の年を考えるのよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("pat1", "差し引きに使えるのは、小さいほうの10万円までなのだ。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("amari", "だから差は、20315円になるの。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("amari", "そして余った損の20万円は、消えて終わりなのだ。", anim=0.0,
         face="troubled", speed=1.05, chara="none"),
    Unit("pat2", "では逆に、損が10万円で利益が30万円の年はどうなのだ?", anim=1.4,
         speed=1.05, chara="none"),
    Unit("pat2", "差し引きに使えるのも、小さいほうの10万円まで。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("pat2", "だから差はやはり、20315円なのよ。", anim=0.0,
         speaker=2, se="pop", speed=1.05, chara="none"),
    Unit("kousiki", "どちらの年も20315円。差を決めるのは、小さいほうなの。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("nashi", "では課税口座に利益が無い年は、どうなるのだ?", anim=1.4,
         speed=1.05, chara="none"),
    Unit("nashi", "利益が無い年は、引く相手がそもそもいないの。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("nashi", "だから課税口座でも、税金は0円よ。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("nashi", "0円どうしなので、差も0円になるのだ。", anim=0.0,
         se="pop", face="happy", speed=1.05, chara="none"),
    Unit("sorotta", "だから差が出るのは、2つがそろった年だけなの。", anim=1.4,
         speaker=2, speed=1.05, pad=0.6),
    Unit("pat1", "つまり余分に払うのは、小さいほうの2割ほどなのだ。", anim=1.4,
         se="don", speed=1.05, chara="none"),

    # ===== 第4章 自分の場合
    Unit("ch4", "では第4章。自分の年の明細から、余分の額を拾いにいくわよ。", anim=1.4,
         speaker=2, face="happy", se="pop", speed=1.1),
    Unit("jibun", "まずNISAの損と、課税口座の利益をならべるの。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("jibun", "そして小さいほうに、0.20315をかけるのよ。", anim=0.0,
         speaker=2, speed=1.05),
    Unit("jibun2_a", "たとえばボクのNISAの損が、4万円だとするのだ。", anim=1.4,
         speed=1.05, chara="none"),
    Unit("jibun2_b", "同じ年の課税口座の利益が、12万円だとするわね。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("jibun2", "その小さいほうは、損の4万円のほうなのだ。", anim=1.4,
         speed=1.05, chara="none"),
    Unit("jibun2", "だから余分に払うのは、8126円になるの。", anim=0.0,
         speaker=2, se="impact", speed=1.05, chara="none", face="surprised"),
    Unit("jibun2", "余分の計算、かけ算だけでいいのだ。", anim=0.0,
         face="happy", speed=1.05, chara="none"),
    Unit("te1", "そして使うときの手順も、4つだけ押さえておくわ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("te1", "まず1つめ。損益通算を使えるのは、同じ年の中だけ。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("te2", "そして2つめ。繰越控除には、確定申告が要るの。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("te2", "その源泉徴収ありの口座でも、申告は要るのだ?", anim=0.0,
         face="troubled", speed=1.05, chara="none"),
    Unit("te3", "申告は要るわ。そして3つめ。NISAで売っても、税の計算は動かない。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("te3", "だから売る時期を、税金で決めなくていいのだな。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("te4", "そして4つめは、売ったあとの投資枠の話よ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("te4", "投資枠とは、NISAで買える金額の上限のこと。", anim=0.0,
         speaker=2, speed=1.05, chara="none"),
    Unit("te4", "売って戻る投資枠は、翌年にならないと使えないの。", anim=0.0,
         speaker=2, face="troubled", speed=1.05, chara="none"),
    Unit("waku2", "たとえば100万円で買ったものが、値下がりしたとするのだ。", anim=1.4,
         speed=1.05, chara="none"),
    Unit("waku2", "80万円で売っても、翌年に戻る投資枠は100万円ぶんなのだ。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("kawaru", "ちなみにこの制度は、2026年8月時点の内容なの。", anim=1.4,
         speaker=2, speed=1.05),
    Unit("kawaru", "制度は毎年変わるから、見る時期に応じて確かめてね。", anim=0.0,
         speaker=2, face="troubled", speed=1.05),
    Unit("matome", "では3つの場合を、表にまとめるわよ。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("matome", "まず損だけの年は、どちらも0円で同じなのだ。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("matome2", "そして利益だけの年は、40630円の得になるの。", anim=1.4,
         speaker=2, speed=1.05, chara="none"),
    Unit("matome3", "でも損と利益がある年は、40630円の損になるのだ。", anim=1.4,
         speed=1.05, chara="none", pad=0.6, face="troubled"),
    Unit("matome3", "つまり得も損も、同じ40630円なのよ。", anim=0.0,
         speaker=2, se="don", face="surprised", speed=1.05, chara="none"),
    Unit("matome3", "40630円は向きがちがうだけで、額は同じなのだな。", anim=0.0,
         speed=1.05, chara="none"),
    Unit("shime", "だからNISAで売っても、税金の話は動かないのよ。", anim=1.4,
         speaker=2, speed=1.05, face="smug"),
    Unit("tsugi", "税金の20.315%は、概要欄のショート版でも見られるのだ。", anim=1.4,
         face="happy", speed=1.05),
    Unit("tsugi", "ショート版なら、利益と手取りの図だけをさっと見られるわ。", anim=0.0,
         speaker=2, speed=1.05),
    Unit("shime", "売る損得の判断に、税金は持ちこまなくていいのだ。", anim=1.4,
         face="happy", speed=1.05),
]
