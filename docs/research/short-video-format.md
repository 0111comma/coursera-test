> **金融チャンネルの最新指定（2026-09-08／U23〜U25）：** 下側はタイトル・チャンネル登録欄用に空け、右側専用の余白は設けない。FZ002では下360pxを空け、左右の内側余白を同じ32pxにする。360pxは今回の実装値であり、過去に本人が指定したpx値とは主張しない。旧資料の右端UIレール回避は、この金融チャンネルには適用しない。内容は実在商品と実在企業で説明し、X/Y各50%の架空例だけに戻さない。
...
- **R9 セーフエリア**: 金融チャンネルはU23を適用し、下側を空け、右側専用余白は作らない。従来の「右端(ボタン列)・最下部(タイトルUI)に重要情報を置かない」は他シリーズの旧仕様。
