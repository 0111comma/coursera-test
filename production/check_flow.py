...
GYAKUSETSU = ("でも", "しかし", "ところが", "けれど", "だが", "とはいえ",
              "とはいっても")   # 口語形。2026-08-30
...
CONNECTIVES = (
    ...
    "とはいえ", "とはいっても",   # 逆接の受け(口語形も同役。2026-08-30)
    # 話題の転換(2026-09-05)。ユーザー指摘「英語を直訳したものに見える」への対応で
    # 「で、」を外したところ、**話題を変える正規の接続詞**を持っていないと繋げられなかった。
    # 「さて」「ところで」は日本語の転換の接続詞で、接続語を貼る誤魔化しではない
    "さて", "ところで", "話を",
    # 話し言葉の受け(2026-09-04。ユーザー指摘「まだ日本語がAI感強い」)。
    # 「では」「まず」「一方」「つまり」で受けると、ゲートは通るが講義口調になる。
    # 話し言葉で前を受ける形をここに足して、書き手がそちらを選べるようにする
    "で、", "でさ", "てか", "つーか", "じゃあさ", "ってことは", "そしたら", "そんで",
    "だって", "ほら", "いや", "まあ", "それで", "なら", "だったら", "ていうか",
)
...
def check_video(vdir: Path):
    ...
    for i in range(1, last):
        ...
        # 2026-09-07 ユーザー指摘:
        #   「ここの接続詞『でも』おかしくない? 絶対そのまま翻訳してるよね? 直訳。ダメだよ。」
        #
        # 落ちなかった理由: 判定が `COUNTER.search(cur) or shared` で、
        # **現在の文に打ち消しの語(ない/しか等)があれば通っていた**。
        # 日本語の文はたいてい否定語を含むので、この or は事実上の素通りだった。
        #
        # **共通語を必須(and)にする案は捨てた。**公開済みの S027 の
        #   「年金は待つほど増えると、聞いたはず。」→「でも何歳で得かは、誰も計算しない。」
        # が落ちる。これは〈聞いて知っている〉と〈誰も計算していない〉の本物の逆接で、
        # 共通の名詞が無いだけ。**逆接が成り立っているかは前の文の中身を読まないと
        # 決まらないので、正規表現の仕事ではない**(1件でも誤検出を出すゲートは、
        # 次から人が理由なく免除を足す入口になる)。
        # 判断は日本語パネルの「翻訳調ハンター」に渡した
        # (.claude/workflows/nihongo.js に、文頭の逆接が前の文の何を打ち消して
        #  いるかを言えないものは全部 finding にする、という観点を足した)。
        if head in GYAKUSETSU and not (COUNTER.search(cur) or shared):
            breaks.append((i + 1, prev, cur,
                           f"「{head}」なのに、この文が何も打ち消していない。"
