...
# **ただし役割色の「濃いほう」をそのまま持ってくると字が読めなくなる。**
# (2026-08-30。厳格審査 craft/artdirection/nihongo が3人とも独立に指摘)
# テロップは「塗り+濃縁(TELOP_EDGE_EMPH)」で描くので、字形を作っているのは
# **塗りと縁の明度差**であって、地とのコントラストではない。実測:
#     濃緑 #4d7a33 vs 縁 = 1.97 / 濃赤 #b32020 vs 縁 = 1.49
#     旧オレンジ #ffb020 vs 縁 = 5.45(だから読めていた)
# 濃色にした結果、画数の多い字(「開」)は内部の白が縁に飲まれて塊になり、
...
_KETA_RE = re.compile(r"(?<![\d,])\d{4,}(?![\d,]|年)")   # 「1900年」は年号なので区切らない(2026-09-04)
