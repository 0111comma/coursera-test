...
# 2026-09-04: 「プロットが面白くない」(Z001)の再発防止。
# 面白さの審査4人が一致して指したのは「上司の機嫌は変えられない、は誰でも知っている」。
# 誰でも知っていることを丁寧に説明する台本は、文を整えても退屈になる。
# 企画の段階で**この動画の「知らなかった」**を一言で書けないなら、その企画は立っていない。
SHIRANAKATTA_HEAD = re.compile(r"\*\*?この動画の「知らなかった」\*\*?|^#+\s*[0-9.]*\s*この動画の「知らなかった」", re.M)
KNOWN_MARKERS = ("誰でも知って", "常識", "当たり前", "よく言われ")
...
def check_video(vdir: Path):
    ...
    # Z 番台(心理学チャンネル)は「知らなかった」を一言で要求する
    if vdir.name.startswith("Z"):
        shira = _line_after(text, SHIRANAKATTA_HEAD)
        if not shira:
            issues.append(("plan.md", "「知らなかった」が無い",
                           "「**この動画の「知らなかった」**」を1行書くこと。"
                           "視聴者がまだ知らない事実・角度が一言で言えないなら、"
                           "その企画はどれだけ整えても退屈になる(shiteki-log 2026-09-03)"))
        elif any(k in shira for k in KNOWN_MARKERS):
            issues.append(("plan.md", "「知らなかった」が既知",
                           f"「{shira[:40]}」は視聴者が知っていること。"
                           "知らない事実・角度を書くこと"))
