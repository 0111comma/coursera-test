...
YASASHII = {
    ...
    "ヒント", "ボルドー",   # 普通のカタカナ語と、直後に「高等法院」が続く地名
}
    ...
    for i, s in enumerate(subs, 1):
        for w in set(KATAKANA.findall(s)) | set(SHOMEI.findall(s)):
            if w in YASASHII or w in ex or w in seen:
                continue
            seen[w] = i
            # **初出の名前に「あの/その/この」を付けない**(2026-09-08 批評パネル)。
            # 「あの暴君ネロの先生ね」は、肩書きを言っていても
            # 「あなたも知ってるあのネロ」と同意を求める形なので、
            # ユーザー指摘(前に話した前提で話すな)がそのまま残る
            # 「あの暴君ネロ」のように**間に修飾語が挟まる**形も見る(指示語→名前が4字以内)
            for shi in ("あの", "その", "この", "例の"):
                j = s.find(shi)
                if j != -1 and 0 <= s.find(w) - (j + len(shi)) <= 4:
                    issues.append((f"#{i}", w,
                                   f"初めて出す「{w}」に「{shi}」が付いている: 「{s}」。"
                                   f"**知っている前提の言い方**。初出は指示語なしで肩書きから渡すこと"))
                    break
