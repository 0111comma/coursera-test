...
判定(Z 番台の plan.md と render.py に対して):
  ...
  4. 「誰がどう評価した」の行の太字の語のうち、少なくとも1つがナレーションに出てくる

**「なぜ残った」は、企画書には要るが、動画で言わなくてよい**(2026-09-08 U22)。
> 「出典がなぜ残ってるのかな話必要かな? 現代の悩みと昔の哲学者がそれに対して
>   何を言っているのか結びつけるのはとてもいいと思うんだけどあと必要?
>   面白くない箇所増やさなくていいよ」

伝来(写本・修道院・校訂・出版)は、この動画の芯である
「いまの悩み ↔ 昔の哲学者の答え」の線に乗らない。**このゲートが
ナレーションでも言えと要求していたせいで、Z003 は8カット、Z002 は5カットを
書誌に使った。**調べたことは plan.md に残し(嘘を書かない担保になる)、
動画では、それ自体が今夜の行動に効くときだけ1〜2カットで触れる。
...
def check_video(vdir: Path):
    ...
    for q, (ans, _src) in rows.items():
        ...
        if q == "誰が言った":
            ...
            pass          # 企画書には要るが、ナレーションでは言わなくてよい(U22)
