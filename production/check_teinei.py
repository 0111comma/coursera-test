...
判定:
  ...
  1. 尺が短い    : 推定尺が MIN_SEC 未満。**WARNのみ**(2026-08-23に不合格から降格)
                   下限で落とすと、尺に届かせるために内容を足すことになる
...
JARGON = [
    # 「相殺」は L001 のナレーションに2回・画面文字を含めると視聴者が目にするのは8回。
    # それなのに意味を一度も言っていなかった(2026-08-22の調査で発覚。納品済みの見逃し)
    "相殺",
...
EVERYDAY = [
    ...
    "株", "値段", "物価", "円安", "円高",
    # 生活サービスの実名(2026-08-24)。strategy.md §6.2 で名指しを解禁したので、
    # 掴みに使えるようにした。**「標準報酬月額」より圧倒的に日常語**で、
    # 動画配信の利用は30代で46.2%(LINEリサーチ 2025年10月)。
    # 自分の明細にある語なので、1秒で「自分に関係ある」と判定できる。
    "サブスク", "Netflix", "Spotify", "Amazon", "YouTube", "携帯代", "通信費",
]
...
# ---- チャンネル「ヤケに心理学に詳しいずんだもん」(Z 番台)の日常語 ----
# 上の EVERYDAY は**お金のチャンネル用**に書いた一覧で、給料・税金・銀行しか見ていない。
# 心理学チャンネルの1文目に「給料」を要求すると、場面のほうが壊れる。
# Z001 はこれを gate_exempt で逃がしていたが、免除は**次の動画で誰も止めない**ので、
# チャンネルごとの一覧に作り直した(2026-09-07。exempt の TODO どおり)。
# 基準は EVERYDAY と同じ: **興味のない人でも知っている、自分の生活にある物の名前**。
EVERYDAY_Z = [
    "上司", "同僚", "部下", "先輩", "後輩", "同期", "社長", "先生", "親", "友だち", "友達",
    "会議", "打ち合わせ", "予定", "カレンダー", "スケジュール", "残業", "休み", "休日",
    "電車", "帰り道", "通勤", "職場", "会社", "仕事", "メール", "返信", "スマホ", "時間",
    "机", "家", "夜", "朝", "週末", "今週", "来週", "一日", "1日",
]
TOPIC = EVERYDAY
...
def check_video(vdir: Path):
    ...
    everyday = EVERYDAY + EVERYDAY_Z if vdir.name.startswith("Z") else EVERYDAY
    ...
    # **下限で落とさない**(2026-08-23)。ユーザー指示:
    #   「伸びない時は尺を短くし、視聴者が飽きないように求めている部分を凝縮して
    #     動画の密度(テンポ)を上げることが重要である」
    # 長尺で「8分以上」の下限を外したのと同じ理由。下限を不合格にすると、
    # **尺に届かせるために内容を足す**ことになり、密度が下がる。
    # MIN_SEC=43 の根拠も「55秒超は視聴率下位」という**上限側の実測**であって、
    # 下限側は測っていない。だから WARN で出すだけにする。
    if est < MIN_SEC:
        issues.append(("(動画全体)", "尺が短い(WARN)",
                       f"推定{est:.0f}秒。端折っていないかだけ確かめること。"
                       f"**短いこと自体は欠点ではない**(密度を上げた結果ならそれでよい)"))
